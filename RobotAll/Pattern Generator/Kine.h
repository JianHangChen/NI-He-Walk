...
public:
	...
	// 計算最佳化的腳底接觸力
	void clacContactForce();
	void get_linear_angular_momentum(double* result_lm,double* result_am );
	// 計算累積慣量&質量
	void clacJaAM( double * Ia, double *z, double *ra, double * r_sup_foot ,double ma, double * r,double *result_JaAM_col );
	void clacIa2Link(double ma1, double ma2, double m,double *ra1,double * ra2,double* r,double * Ia1,double * Ia2, double * I,YMatLite *T, double * result_ra, double *result_ma,double *result_Ia);
	void clacIa(double ma ,double m,double *ra,double* r,double * Ia, double * I,YMatLite *T, double * result_ra, double *result_ma,double *result_Ia);
	
	void RIRt(double *R, double *I ,double * result);
	void TITt(YMatLite *T, double *I ,double * result);
	void accumulationInertia(double * Ia, double * I,double *R, double ma, double m, double * rda,double *rd, double * result);
	void accumulationInertia2Link(double * Ia1,double * Ia2, double * I,double *R, double ma1,double ma2, double m, double * rda1,double * rda2,double *rd, double * result);
	// 計算累積質心位置 
	void clacCOM( int mass_com_index,  int N , double* result_ra, double *result_ma );
	void accumulationCenterOfMass( double ma, double m, double *ra, double *r,double *result_ma, double *result ,double * result_rda, double * result_rd );
	void accumulationCenterOfMass2Link( double ma1, double ma2,double m, double *ra1, double *ra2,double *r,double *result_ma, double *result,double * result_rda1,double * result_rda2, double * result_rd  );
	void Rrpy( double  r, double p,double y,double * result );
	// 計算COG Jacobian
	void GetCOGJacobian(void);
	// 計算AngularMomentum Jacobian
	void GetAngularMomentumJacobian(void);
	// 計算COG 位置
	void FindCOG(void);
	// 解一格IK
	...
	void IMUSensorData(int LogCount,float *accex ,float *accey,float *accez, float *angvelx,float *angvely,float *angvelz, float *roll, float *pitch, float *yaw, float *rollF, float *pitchF, float *yawF, float *pos_x, float *pos_y, float *pos_z);
	...
	double **inertia;//==>  桿件質量 YJ 2015/01/30 目前只使用手臂肩膀的翻滾方向的自由度
	/*double **angularMomentumJa;*/
	YMatLite* AMJa; //  angular Momentum Jacobian matrix
	int AMJaMRow; // Jacobian matrix 的row數
	int AMJaNCol; // Jacobian matrix 的column數
	double J_LL[36];
	double J_RL[36];
	double ZMP[2];
	int selIK; // 左右腳支撐狀態
