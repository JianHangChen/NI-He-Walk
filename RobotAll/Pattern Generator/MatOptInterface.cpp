...
void showMat(double* A,int Nrow, int Ncol )
{
	...
	for (int i = 0 ; i < Nrow ; i++)
	{
		for (int j = 0 ; j < Ncol ; j++)
		{
			...
			if (A[j+i*Ncol] == 0)
				cout<< "\t" << "0" ;
			else if (fabs(A[j+i*Ncol]) < 0.000001)
				cout << "\t"<< "0.0" ;
			else
				cout << "\t"<< A[j+i*Ncol] ;
		}
...
{
	cout << "-------------------Varible Data:---------------------- " << endl;
	cout<<"\t"<<x<<endl;

}
void Mat_svd ( double* J, integer Jrow, integer Jcol,  double *U, double *S, double *Vt)
{
